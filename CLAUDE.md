...
8. 作業の振り返りを行う（下記「Issue 完了時の振り返り」を参照）
```

#### Issue 完了時の振り返り

Issue が完了したら必ず以下を行うこと：

1. **振り返りドキュメントを作成する**
   - `notes/work-logs/issue-番号-振り返り.md` に以下を記録する
     - 目的：何のためにやったか
     - やったこと：具体的な作業内容
     - ハマった点：何が問題だったか・なぜハマったか・どう解決したか
     - 学んだこと：次に活かせること

2. **学習メモを更新する**
   - ハマりポイントや技術的な学びは `notes/learnings/` の該当ファイルに追記する

3. **詳細設計ドキュメントを修正する**
   - 実装中に設計から変更になった点があれば `docs/design/` を更新する
   - 次の Issue のために設計が不足している部分があれば補足する
